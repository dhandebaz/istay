  ...
  if (loading) {
    return (
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 animate-pulse">
        {[1, 2, 3].map((i) => (
          <div key={i} class="bg-gray-100 rounded-[2.5rem] aspect-video sm:aspect-square" />
        ))}
      </div>
    );
  }
  ...
  if (properties.length === 0) {
    return (
      <div class="text-center py-20 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
        ...
        <p class="text-gray-400 text-sm mt-2">Check back soon for curated direct bookings.</p>
      </div>
    );
  }
        ...
        >
          ...
          <div class="aspect-square overflow-hidden relative">
            ...
            <div class="absolute top-6 right-6 px-4 py-2 rounded-2xl bg-white/90 backdrop-blur-md shadow-lg">
              <span class="text-istay-900 font-900 text-sm">₹{prop.basePrice.toLocaleString()}</span>
              <span class="text-gray-500 text-[10px] font-700 uppercase tracking-tighter ml-1">/night</span>
            </div>
