    ...
    <div class="relative bg-white rounded-3xl border border-gray-200 shadow-2xl overflow-hidden h-[340px] flex flex-col">
      ...
      <div class="flex-1 relative flex flex-col items-center justify-center p-6 bg-white overflow-hidden">
        ...
        <div class={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 ease-out ${step === 0 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
          ...
          <p class="text-sm font-500 text-gray-400 mt-1">We'll handle the rest...</p>
        </div>

        {/* Step 1: Scanning */}
        <div class={`absolute inset-0 p-6 flex flex-col justify-center transition-all duration-500 ease-out ${step === 1 ? 'opacity-100 translate-x-0' : step < 1 ? 'opacity-0 translate-x-8' : 'opacity-0 -translate-x-8'}`}>
          <div class="flex items-center gap-3 mb-4">
             <div class="w-8 h-8 rounded-full bg-rose-500 text-white flex items-center justify-center text-xs font-800">A</div>
        ...
        {/* Step 3: Success */}
        <div class={`absolute inset-0 p-6 flex flex-col justify-center transition-all duration-500 ease-out ${step === 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}>
          <div class="bg-gradient-to-br from-istay-900 to-istay-800 rounded-2xl p-5 text-white flex flex-col shadow-xl border border-istay-700/50 h-full">
            <div class="flex justify-between items-center mb-6">
              <span class="text-xs font-900 tracking-wider flex items-center gap-1.5">
                 <img src="/logo.svg" alt="istay" class="h-4 brightness-0 invert" />
              </span>
