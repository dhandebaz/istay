...
import type { ChatSession, ChatMessage, GuestProfile } from "../../../utils/types.ts";
...
`.trim() : "- No historical data for this guest yet."}
    ...
    function_declarations: [
      {
        ...
        description: "Retrieves specific house rules, WiFi, or gate codes from the property knowledge base.",
        parameters: {
          type: "OBJECT",
          properties: {
            topic: { type: "string", description: "The topic to lookup (e.g. 'WiFi', 'Gate Code')" },
          },
        },
      },
      {
        name: "create_supply_request",
        description: "Logs a formal supply or maintenance request for the host/caretaker.",
        parameters: {
          type: "OBJECT",
          properties: {
            item: { type: "string", description: "The item or service requested (e.g. 'water', 'towel')" },
      ...
      },
      {
        ...
        description: "Updates the guest's ID verification status after reviewing submittted documents.",
        parameters: {
          type: "OBJECT",
          properties: {
            isVerified: { type: "boolean", description: "Whether the ID is approved" },
      ...
      },
      {
        ...
        description: "Suggests alternative luxury stays across the iStay platform when current dates are blocked.",
        parameters: { type: "OBJECT", properties: {} },
      },
      {
        name: "get_live_availability",
        description: "Checks if the property is available for a specific date range.",
        parameters: {
          type: "OBJECT",
          properties: {
      ...
      },
      {
        ...
        description: "Connects the guest to a human host for complex issues or disputes.",
        parameters: {
          type: "OBJECT",
          properties: {
...
export const handler: Handlers = {
  ...
  POST: async (req) => {
    try {
      ...
      if (msg.type === "image") {
        console.log(`[whatsapp] Image received from ${guestPhone} (id: ${msg.image.id})`);
      ...
      const systemPrompt = buildSystemPrompt(property.name, knowledgeContent, guestProfile || undefined);
      ...
      if (msg.type === "image" && booking) {
        ...
        if (b64) {
          await sendWhatsAppMessage(guestPhone, "Got it! Reviewing your ID now... 🧐");
          
          try {
            const ocrResponse = await callGeminiVision({
              imageBase64: b64, 
              prompt: `Extract ID details. Compare against Guest Name: "${booking.guestName}". 
            ...
            if (ocrResult && ocrResult.match_score > 85) {
              ...
              await sendWhatsAppMessage(guestPhone, `✅ verification successful, ${booking.guestName}! You're all set for check-in.`);
              // Short-circuit the text logic
              return new Response("OK", { status: 200 });
            } else {
              await sendWhatsAppMessage(guestPhone, "Hmm, I couldn't verify that ID perfectly. Please ensure the photo is clear and matches your booking name! 📷");
            }
          } catch (e) {
        ...
        try {
          ...
          if (response.toolCalls && response.toolCalls.length > 0) {
            ...
            if (tool.name === "get_stay_details") {
              ...
              toolResult = { status: "success", message: `Caretaker notified about ${item}.` };
              ...
              const isBlocked = blocks.some(b => b.date >= start && b.date < end);
              ...
              if (isBlocked) {
                ...
                toolResult = { 
                  status: "success", 
                  available: false,
                  message: "Dates are already booked. However, I found these similar luxury stays for you!",
                  recommendations
                };
              } else {
                toolResult = { 
              ...
              toolResult = { status: "success", current_status: booking.status, checkIn: booking.checkIn };
              ...
              toolResult = { status: "success", message: "Human host notified." };
            }
